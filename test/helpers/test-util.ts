...
export default class TestUtil {
  ...
  static readonly TABLE_SEPARATOR_CHAR: string = '│' // note: this is not a pipe "|" character, but something that looks extremely like it

  static validateTablePrintout(output: string, printouts: SoftwarePrintout[]): void {
    const lines = output.split(/\\\\r\\\\n?|\\\\n|\\r\\n?|\\n|\r\n?|\n/)
    const header = lines[1]
    const headerCols = header.split(TestUtil.TABLE_SEPARATOR_CHAR)
    ...
    for (let i = 0; i < printouts.length; i++) {
      ...
      const cols = line.split(TestUtil.TABLE_SEPARATOR_CHAR)
